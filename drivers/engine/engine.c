...
{
	// pr_info("motor 1 callback 함수: jiffies 값: (%ld).\n", jiffies);
	...
	hrtimer_forward_now(timer, ms_to_ktime(MAX_TIMEOUT_MSEC - (MAX_TIMEOUT_MSEC * motor_1_speed / 100 + 1)));
	return HRTIMER_RESTART;
}

enum hrtimer_restart motor_2_timer_callback(struct hrtimer *timer)
{
	// pr_info("motor 2 callback 함수: jiffies 값: (%ld).\n", jiffies);
	...
	hrtimer_forward_now(timer, ms_to_ktime(MAX_TIMEOUT_MSEC - (MAX_TIMEOUT_MSEC * motor_2_speed / 100 + 1)));
	return HRTIMER_RESTART;
}
...
static int __init toy_module_init(void)
{
	...
	if (request_irq(button_irq, (irq_handler_t) k_gpio_irq_signal_handler, IRQF_TRIGGER_RISING, "k_gpio_irq_signal", NULL) != 0) {
