...
BUILDROOT_DIR = /home/sungho/kdt/buildroot
SYSROOT = $(BUILDROOT_DIR)/output/host/aarch64-buildroot-linux-gnu/sysroot
...
CSRC = system/main.c system/ui/gui.c system/ui/input.c system/web_server/web_server.c \
       system/system/system_server.c system/system/shared_memory.c system/system/dump_state.c \
       system/hal/hardware.c 
CXXSRC = system/engine/Allocator.cpp system/engine/Engine.cpp \
		 system/engine/LeftMotor.cpp system/engine/RightMotor.cpp system/engine/StateMachine.cpp \
		 system/engine/stdafx.cpp system/engine/xallocator.cpp
...
# Default target
all: toy-fe toy-be $(TARGET) libcamera.oem.so libcamera.toy.so
...
toy-be: | ; $(info $(M) building toy be…)
	cd toy-be && env CC=$(CC) CXX=$(CXX) CGO_CFLAGS_FOR_TARGET="--sysroot=$(SYSROOT)" CGO_LDFLAGS_FOR_TARGET="--sysroot=$(SYSROOT)" GOARCH=arm64 CGO_ENABLED=1 GOOS=linux GOGC=off GO111MODULE=on  $(GO) build -ldflags '$(GO_LDFLAGS)' -o toy-be
