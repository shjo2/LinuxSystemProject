...
void *engine_thread(void* arg)
{
    ...
    while (1) {
        ...
        assert(mqretcode >= 0);
        printf("engine_thread: 메시지가 도착했습니다.\n");
        printf("msg.id: %d\n", msg.id);
        printf("msg.data.motor.id: %d\n", msg.data.motor.id);
        printf("msg.data.motor.speed: %d\n", msg.data.motor.speed);
#if 1
        if (msg.id == MESSAGE_ID_ENGINE && msg.data.motor.id == 0) { // Left motor
            if ( msg.data.motor.speed == 0) { // Halt
                halt_left_motor();
            } else {
                set_left_motor_speed(msg.data.motor.speed);
            }
        } else if (msg.id == MESSAGE_ID_ENGINE && msg.data.motor.id == 1) { // Right motor
            if ( msg.data.motor.speed == 0) { // Halt
                halt_right_motor();
            } else {
            ...
            printf("engine_thread: unknown message. xxx\n");
        }
#endif
    }
