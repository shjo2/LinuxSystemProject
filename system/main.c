...
int main()
{
    ...
    assert(retcode == 0);
    retcode = create_message_queue(&engine_queue, "/mq_be_to_sys", NUM_MESSAGES, sizeof(robot_message_t));
    assert(retcode == 0);
    retcode = create_message_queue(&backend_queue, "/mq_sys_to_be", NUM_MESSAGES, sizeof(robot_message_t));
    assert(retcode == 0);
